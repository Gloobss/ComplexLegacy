  ...
  return (
    <footer className="relative bg-gta-graphite border-t border-gta-medium">
      <div className="container-gta py-12">
        {/* Grid principal: 1 col en móvil, 3 cols desde md */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8 items-start">
          
          {/* Server Info — asegurado visible en todos los breakpoints */}
          <div className="block col-span-1 min-w-0">
            ...
            {siteConfig.server.tagline && (
              <p className="text-gta-light/90 font-inter text-sm mb-2">
                {siteConfig.server.tagline}
              </p>
            )}
            ...
            {/* Tips opcionales: se muestran si existen, en todos los tamaños */}
            {Array.isArray(siteConfig.server.loadingTips) && siteConfig.server.loadingTips.length > 0 && (
              <ul className="mt-4 space-y-1 list-disc list-inside text-gta-light/90 text-sm">
                {siteConfig.server.loadingTips.slice(0, 4).map((tip: string, i: number) => (
                  <li key={i}>{tip}</li>
                ))}
              </ul>
            )}
